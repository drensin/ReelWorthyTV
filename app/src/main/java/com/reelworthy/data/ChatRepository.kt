 ...
 * API, including the placement of `thinking_config` within `generationConfig` for reasoning models.
 * 3. **Streaming & Parsing**: Managing the SSE (Server-Sent Events) stream, parsing raw JSON
 * chunks, and extracting both "thinking" content and the final structured JSON response.
 *
...
class ChatRepository(
     ...
     * 4. Builds the JSON request body manually using [JSONObject]/Maps to ensure correct structure,
     * ```
     ...
     *
     * @param userQuery The user's natural language search query (e.g., "Show me something funny").
    ...
    fun getRecommendations(userQuery: String): Flow<ChatStreamUpdate> =
        ...
        """.trimIndent()
                        ...
                        val generationConfig =
                                mutableMapOf<String, Any>("responseMimeType" to "text/plain")
                        ...
                        try {
                            ...
                            val mediaType =
                                    OkHttpUtils.createMediaType("application/json; charset=utf-8")
                            val requestBodyObj =
                                    OkHttpUtils.createRequestBody(mediaType, jsonPayload)
                            ...
                            if (!response.isSuccessful) {
                                ...
                                Log.e("ChatRepository", "API Error Body: $errorBody")
                                                ...
                                                for (i in 0 until parts.length()) {
                                                    ...
                                                    if (part.has("thought") &&
                                                        ...
                                                        if (accumulatedText.contains("```json")) {
                                                            display +=
                                                                    accumulatedText.substringBefore(
                                                                            "```json"
                                                                    ) +
                                                                            "\n\n[Receiving structured results...]"
                                                        } else {
                                                            display += accumulatedText
                                                        }
                                                        emit(ChatStreamUpdate(display, false))
                                                    }
                                                }
